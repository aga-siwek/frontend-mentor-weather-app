...
function TodayWeatherInfo() {
  ...
  const feelsLikeCalculation = () => {
    if (apiWeatherInfo?.current) {
      if (measureType === "METRIC") {
        const metricFeelsLike = Math.round(
          apiWeatherInfo.current.apparent_temperature,
        );
  ...
  const windSpeedCalculation = () => {
    if (apiWeatherInfo?.current) {
      if (measureType === "METRIC") {
        const metricWindSpeed = Math.round(
          apiWeatherInfo.current.wind_speed_10m,
        );
  ...
  const precipitationCalculation = () => {
    if (!apiWeatherInfo?.current) {
      ...
      const metricPrecipitation = Math.round(
        apiWeatherInfo.current.precipitation,
      );
  ...
  return (
    <div className={styles.today_weather_info_container}>
      ...
      <TodayWeatherInfoBox
        boxHeader="Precipitation"
        boxResult={precipitation}
      />
    </div>
  );
}
