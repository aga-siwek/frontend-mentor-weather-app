...
function TodayMainInfo() {
  ...
  if (apiWeatherInfo) {
    ...
    console.log("Weather info", apiWeatherInfo, false);
  }
  ...
  const getIcon = () => {
    ...
    if (todayWeatherCode === 0)
      return (
        <img
          src={todayIsDay ? sunnyIcon : overcastIcon}
    ...
    if (todayWeatherCode === 1 || todayWeatherCode === 2)
      return (
        <img
          src={todayIsDay ? partlyCloudyIcon : overcastIcon}
          className={styles.icon_svg}
          alt="partly cloudy"
        />
      );
    ...
    <div className={styles.today_temp_container}>
      <div className={styles.main_info}>
        <div className={styles.main_info_localization}>
          <p className={styles.main_info_localization_text}>
            {countryCalculation()}, {cityCalculation()}
          </p>
        </div>
        <div className={styles.main_info_date}>
          <p
            className={styles.main_info_date_text}
          >{`${dayName}, ${todayDay} ${monthsName[todayMonth]} ${todayYear} `}</p>
        </div>
      </div>
      <div className={styles.temperature}>
        <div className={styles.temperature_icon}>{getIcon()}</div>
        <div className={styles.temperature_info}>
          <p className={styles.temperature_info_text}>
            {" "}
            {todayTemperature} &#176;
          </p>
        </div>
